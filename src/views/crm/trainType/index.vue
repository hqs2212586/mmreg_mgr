        ...
        <tree-table v-loading="loading" :data="train_types" :expand-all="true" :columns="columns" border size="small">
          <el-table-column label="操作" width="150px" align="center">
            <template slot-scope="scope">
              <edit v-if="checkPermission(['admin','traintype_all','traintype_edit'])" :train_types="train_types" :data="scope.row" :sup_this="sup_this"/>
              <el-popover
                v-if="checkPermission(['admin','traintype_all','traintype_delete'])"
                :ref="scope.row.id"
                placement="top"
                width="200">
                <p>确定删除本条数据吗?</p>
